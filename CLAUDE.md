...
- **Document type is metadata, never inference**: `Store/NoteMetadata.swift` (`NoteMetadataStore`) keeps a `.thoughtqueue/metadata.json` sidecar in the store root, keyed by store-relative path, recording each note's `NoteDocType` (`.markdown` / `.checklist`). `NoteStore.docType(of:)` is the one resolver, and content is never consulted: no recorded type means `.markdown`. `TaskList.isChecklist` survives only to decide whether to *offer* the Shift+Cmd+M conversion. This is deliberate and was a bug fix: two notes that are both nothing but `- [ ]` lines can be one person's packing list and another's scratch pad, so no rule over the bytes can tell them apart, and inferring meant typing a checkbox into a scratch note silently converted it on the next open. `createNote(..., docType:)` records it ("+ List" is `.checklist`, "+ Add" and capture are `.markdown`), rename/move/delete/category edits remap or drop the key, and Shift+Cmd+M writes the user's choice. `NoteMetadataStore.key(for:root:)` normalizes both sides itself (symlinks resolved, leading `/private` stripped textually) because `standardizedFileURL` only strips that prefix for a path that still exists, and keys are computed for files that have just moved.
