...
- **Document type is metadata, never inference**: `Store/NoteMetadata.swift` (`NoteMetadataStore`) keeps a `.thoughtqueue/metadata.json` sidecar in the store root, keyed by store-relative path, recording each note's `NoteDocType` (`.markdown` / `.checklist`) and each category folder's archive flag. `NoteStore.docType(of:)` is the one resolver, and content is never consulted: no recorded type means `.markdown`. `TaskList.isChecklist` survives only to decide whether to *offer* the Shift+Cmd+M conversion. This is deliberate and was a bug fix: two notes that are both nothing but `- [ ]` lines can be one person's packing list and another's scratch pad, so no rule over the bytes can tell them apart, and inferring meant typing a checkbox into a scratch note silently converted it on the next open. `createNote(..., docType:)` records it ("+ List" is `.checklist`, "+ Add" and capture are `.markdown`), rename/move/delete/category edits remap or drop the key, and Shift+Cmd+M writes the user's choice. A folder is archived only when `NoteStore.setArchived` records it (a folder named "Archive" is just a name); `visibleNotes()` is what the popover lists, while `allNotes()` stays complete. `NoteMetadataStore.key(for:root:)` normalizes both sides itself (symlinks resolved, leading `/private` stripped textually) because `standardizedFileURL` only strips that prefix for a path that still exists, and keys are computed for files that have just moved.
...
- **Right-click menu bar icon**: Context menu with Categories, Open Notes Folder (reveals the store root in Finder), Preferences, Quit. Built by `AppDelegate.buildContextMenu()` separately from showing it, so its contents are testable; `autoenablesItems` is off because items dispatch up the responder chain.
- **CategoryManagerWindowController**: Singleton window for adding, renaming, archiving, and deleting category folders (delete moves notes to Uncategorized; archive hides the folder's notes from the popover only). Opened from the right-click menu.
...
- **Checkbox lists**: a list note (`NoteStore.docType(of:)` is `.checklist`, which only "+ List" or Shift+Cmd+M ever makes it) is presented by `ChecklistViewController`, a stack of live row views, *instead of* the markdown text view. It is not an edit mode: there is no view/edit toggle on a list note, boxes are always clickable, and each item is its own `NSTextField`. `NoteEditorViewController.ContentMode` (`checklist` / `rendered` / `raw`) is chosen by `desiredMode(honoringStartInEditMode:)`, where checklist outranks `startInEditMode` and ignores the `noteEditMode` preference entirely. View > Edit as Markdown / Edit as Checklist (Shift+Cmd+M, `toggleChecklistMarkdown:`) *records* the new type and sets `forceMarkdownForSession` so the view drops straight to raw; the type outlives the window, the session flag does not.
- **A category change must not blank a list**: `NoteStore.move`/`rename` post `.notesDidChange` from inside the store, i.e. before `applyNoteChange` swaps the new URL into the editor, so for that instant `note.url` names a file that no longer exists. `onNotesChangedExternally` bails out when the file is missing rather than resyncing against an empty read (which used to leave the window blank until it was reopened).
...
- **NoteNavigatorViewController**: The note window's navigation panel. Source-list outline of notes grouped by category with a fuzzy search field; selecting a note retargets the same window via `NoteEditorViewController.load(note:)`, unless that note already has its own window (which is focused via `NoteWindowController.focus`, including deminiaturize and `.moveToActiveSpace`). Archived folders sort last (`NoteStore.navigatorCategories()`) and show a dimmed header plus an archive-box glyph. Right-click a note to set or unset the working document. It loads its tree in `viewDidLoad`, so the panel is never blank even if AppKit puts it on screen before the first explicit `reload()`.
...
`Theme.swift` is the single source of truth for the cream/sage light palette and its dark counterpart (every color is a light/dark `NSColor` pair via `NSColor(light:dark:)`, so it tracks system appearance automatically), plus the bundled Figtree body font (`Theme.body`) and Georgia accent font (`Theme.heading`). `ThemedControls.swift` holds the reusable custom-drawn controls built on those tokens: `ThemedButton` (filled/outlined pill buttons), `ThemedSearchField` (flat pill search input), `SegmentedPillControl` (two-way toggle, e.g. the editor font quick-picker), `ThemedCheckbox` (custom-drawn checkbox used by the checklist rows; drawn in `draw(_:)` rather than `updateLayer()` because it strokes a box and a tick, and its checkmark path is flip-aware). The drawing itself lives in `CheckboxGlyph`, shared with `MarkdownRenderer`'s `CheckboxAttachmentCell` so a checkbox in rendered markdown is the same box as one in a list, not the small `\u{2610}` glyph it used to be. It is an attachment *cell* rather than a baked image so `Theme`'s light/dark pairs resolve on each redraw; `MarkdownRenderer.checkboxPlaceholder` is the two-unit string it occupies (`\u{FFFC} `), the same width as the glyph, so gutter offsets are unchanged. New UI should pull colors/fonts from `Theme` rather than system colors, except where a control's native chrome is left as-is deliberately (e.g. `NSPopUpButton`, native checkboxes, and `NoteNavigatorViewController`'s `NSSearchField`, which tests drive directly).
